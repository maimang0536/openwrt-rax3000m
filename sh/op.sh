...
git_sparse_clone main https://github.com/openwrt/openwrt package/firmware/linux-firmware
git_sparse_clone openwrt-24.10 https://github.com/immortalwrt/immortalwrt package/network/config/wifi-scripts

rm -rf package/kernel/{gpio-button-hotplug,mac80211,mt76}
cp -rf {gpio-button-hotplug,mac80211,mt76} package/kernel
rm -rf package/firmware/linux-firmware
cp -rf linux-firmware package/firmware
...
if [ "$SOURCE_NAME" = "immortalwrt" ]; then
 sed -i 's/option dns_redirect\t1/option dns_redirect\t0/' package/network/services/dnsmasq/files/dhcp.conf
 sed -i "s/system/services/" feeds/luci/applications/luci-app-ttyd/root/usr/share/luci/menu.d/luci-app-ttyd.json
